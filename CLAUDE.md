...
│   ├── openai-converter.js     # OpenAI ↔ Anthropic conversion (for Cursor IDE)
...
- Thinking models fall back to thinking models (e.g., `claude-sonnet-4-5-thinking` → `gemini-3-pro-high`)
...
**OpenAI-Compatible APIs (for Cursor IDE):**

- `/v1/chat/completions` - OpenAI Chat Completions API
  - Converts OpenAI format → Anthropic format → Google format internally
  - Supports streaming (SSE), tool calls, images, and thinking models
  - Response is converted back to OpenAI format
  - Configure Cursor to use `http://localhost:8080` as the OpenAI base URL
