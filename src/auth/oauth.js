...
// Track the current callback server so we can close it before starting a new one
let currentCallbackServer = null;
let currentServerTimeout = null;

/**
 * Close any existing callback server
 * Call this before starting a new OAuth flow to prevent state mismatch errors
 */
export function closeCallbackServer() {
    if (currentServerTimeout) {
        clearTimeout(currentServerTimeout);
        currentServerTimeout = null;
    }
    if (currentCallbackServer) {
        try {
            currentCallbackServer.close();
            logger.info('[OAuth] Closed existing callback server');
        } catch (e) {
 ...
 * 
 * NOTE: If a callback server is already running, it will be closed first.
 * This prevents "state mismatch" errors when starting multiple OAuth flows.
 *
...
export function startCallbackServer(expectedState, timeoutMs = 120000) {
    // Close any existing server first to prevent conflicts
    closeCallbackServer();
    ...
    return new Promise((resolve, reject) => {
        ...
        server.listen(OAUTH_CONFIG.callbackPort, () => {
            ...
            // Track the server so it can be closed if a new flow starts
            currentCallbackServer = server;
        });

        // Timeout after specified duration - track it so we can cancel if needed
        ...
        // Helper to clean up on completion (success or error)
        const cleanup = () => {
            currentCallbackServer = null;
            if (currentServerTimeout) {
                clearTimeout(currentServerTimeout);
