...
import { getAuthorizationUrl as getAuthUrl } from '../auth/oauth.js';
import { getServerState, updateServerState } from '../state.js';
import { configureNgrok, startTunnel } from '../ngrok/tunnel.js';
import { setConfig, generateApiKey, getConfig } from '../setup/env-config.js';
...
function createAuthMiddleware() {
    return (req, res, next) => {
        ...
        const isException = req.path === '/api/auth/url';
...
export function mountWebUI(app, dirname, accountManager) {
    ...
    app.get('/setup', (req, res) => {
        res.sendFile(path.join(dirname, '../public/setup.html'));
    });
    ...
    // ==========================================
    // Cursor Configuration API (NEW)
    // ==========================================

    /**
     * GET /api/cursor/config - Get Cursor-ready configuration
     */
    app.get('/api/cursor/config', async (req, res) => {
        try {
            ...
            logger.error('[WebUI] Error getting cursor config:', error);
            res.status(500).json({ status: 'error', error: error.message });
        }
    });

    /**
     * GET /api/cursor/settings - Get Cursor-specific settings
     */
    app.get('/api/cursor/settings', (req, res) => {
        try {
            const state = getServerState();
            res.json({
                status: 'ok',
                enable1MContext: state.enable1MContext || false
            });
        } catch (error) {
            res.status(500).json({ status: 'error', error: error.message });
        }
    ...
    /**
     * POST /api/cursor/settings - Update Cursor-specific settings
     */
    app.post('/api/cursor/settings', (req, res) => {
        try {
            const { enable1MContext } = req.body;

            if (typeof enable1MContext === 'boolean') {
                updateServerState({ enable1MContext });
                logger.info(`[WebUI] Updated 1M Context Mode: ${enable1MContext}`);
            }
    ...
    /**
     * POST /api/cursor/regenerate-key - Regenerate API Key
     */
    app.post('/api/cursor/regenerate-key', async (req, res) => {
        try {
            const { generateApiKey, setConfig } = await import('../setup/env-config.js');

            const newKey = generateApiKey();

            // Save to .env
            setConfig('PROXY_API_KEY', newKey);

            // Update runtime state
            updateServerState({ apiKey: newKey });

            logger.info('[WebUI] API key regenerated by user');
    ...
    /**
     * POST /api/setup/ngrok - Configure ngrok auth token
     */
    app.post('/api/setup/ngrok', async (req, res) => {
        try {
            ...
            const { setConfig } = await import('../setup/env-config.js');
            const { configureNgrok, startTunnel } = await import('../ngrok/tunnel.js');

            // Configure ngrok
            const success = await configureNgrok(authToken.trim());

            if (!success) {
                return res.status(500).json({
                    status: 'error',
                    error: 'Failed to configure ngrok'
                });
            }

            // Save to .env
            setConfig('NGROK_AUTH_TOKEN', authToken.trim());

            // Try to start tunnel
            try {
                const state = getServerState();
                const url = await startTunnel(state.port);
                updateServerState({ ngrokUrl: url, needsSetup: false });

                res.json({
                    status: 'ok',
                    message: 'ngrok configured and tunnel started',
                    url: url
                });
            } catch (tunnelError) {
                res.json({
                    status: 'ok',
                    message: 'ngrok configured. Restart server to start tunnel.',
                    warning: tunnelError.message
                });
            }
        } catch (error) {
            logger.error('[WebUI] Error configuring ngrok:', error);
            res.status(500).json({ status: 'error', error: error.message });
        }
    });
    ...
    app.get('/api/setup/status', async (req, res) => {
        try {
            const { isNgrokInstalled, isNgrokConfigured, getNgrokStatus } = await import('../ngrok/tunnel.js');
            const state = getServerState();

            // Check accounts
            const accountStatus = accountManager.getStatus();
            const hasAccounts = accountStatus.total > 0;

            // Check ngrok
            const ngrokInstalled = await isNgrokInstalled();
            const ngrokConfigured = await isNgrokConfigured();
            const ngrokStatus = await getNgrokStatus();

            res.json({
                status: 'ok',
                setup: {
                    complete: hasAccounts && ngrokStatus.running,
                    steps: {
                        ngrokInstalled,
                        ngrokConfigured,
                        ngrokRunning: ngrokStatus.running,
                        hasAccounts,
                        accountCount: accountStatus.total
                    }
                },
                serverState: state
            });
        } catch (error) {
            logger.error('[WebUI] Error getting setup status:', error);
            res.status(500).json({ status: 'error', error: error.message });
        }
    });
